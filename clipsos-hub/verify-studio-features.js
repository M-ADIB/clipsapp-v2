...
async function run() {
  console.log("Starting Content Studio Dynamic Docs & Onboarding Verification...");
  ...
  try {
    // -------------------------------------------------------------
    // PART 1: STRATEGIST / OWNER VIEW & ONBOARDING SET UP
    // -------------------------------------------------------------
    console.log("--- PART 1: OWNER SETUP ---");
    ...
    await page.waitForTimeout(2000); // Wait for creation and editor loading
    ...
    await page.waitForTimeout(2000); // Wait for auto-save hook
    ...
    // -------------------------------------------------------------
    // PART 2: CLIENT VIEW & SUBMIT ANSWER
    // -------------------------------------------------------------
    ...
    // -------------------------------------------------------------
    // PART 3: REVOKE ACCESS (UNSEND)
    // -------------------------------------------------------------
    console.log("--- PART 3: REVOKE ACCESS ---");
    console.log("Logging in as Owner to revoke...");
    await page.goto(`${BASE_URL}/login`);
    await page.waitForSelector("#login-email");
    await page.waitForLoadState("networkidle");
    await page.waitForTimeout(2000);
    ...
    await page.waitForTimeout(2000); // Wait for mutation & state update
    ...
    // -------------------------------------------------------------
    // PART 4: VERIFY CLIENT CANNOT ACCESS STUDIO TAB
    // -------------------------------------------------------------
    console.log("--- PART 4: VERIFY CLIENT BLOCKED ---");
    ...
    if (count === 0) {
      ...
      console.error("FAIL: Studio tab is still visible for client!");
    }
    ...
    await page.screenshot({ path: path.join(SCREENSHOT_DIR, "99_test_failure.png") });
  } finally {
    await browser.close();
