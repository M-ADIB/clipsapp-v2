...
## 9. 🔑 Dev Credentials & Browser Testing

> **Canonical location:** `src/components/dev/DevTools.tsx` → `DEV_CREDENTIALS` constant
...
This is an **Owner** account. Once logged in, the app has **DevTools** (floating bug icon, bottom-right corner) that lets you switch to ANY role without re-logging in.

### How to Use

1. Navigate to the running app (usually `http://localhost:8080`)
2. Log in with the credentials above
3. You'll land on the Owner dashboard
4. Click the 🐛 **bug icon** (bottom-right) to open DevTools
5. Use **"Switch role"** to preview any role's dashboard (Manager, Senior Editor, etc.)
6. Use **"Dev Credentials"** section in DevTools to copy email/password

### Important Notes

- DevTools only appear when `tenant.settings.dev_mode = true`
- Role switching is **frontend-only** — RLS still enforces the real (owner) role
- This means you'll see owner-level data regardless of which role you're previewing
- For true role-scoped data testing, use separate accounts per role
