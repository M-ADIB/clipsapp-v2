...
serve(async (req) => {
  ...
  try {
    ...
    // Verify requesting user is allowed to invite (owner or manager)
    const authHeader = req.headers.get("Authorization")!;
    const token = authHeader.replace("Bearer ", "");

    // We could verify the user's role here using supabaseClient.auth.getUser()
    // For now, let's proceed with the admin invite using the service role
