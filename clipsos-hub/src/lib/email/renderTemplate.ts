/**
 * renderTemplate — single source of truth for assembling email HTML
 * from a structured template (Visual mode) OR raw HTML (HTML mode).
 *
 * Used by:
 *  - Email Hub live preview (this codebase)
 *  - Edge Function `email-sender` (Deno twin keeps the same shape)
 *
 * The rule: if edit_mode === "html", we use body_html as-is. Otherwise we
 * compose a styled wrapper around the structured fields (headline, body, cta).
 *
 * Master template support: when a MasterTemplate is provided,
 * content is injected into the wrapper shell instead of building
 * a standalone HTML document.
 *
 * Variable interpolation happens AFTER the wrapper is built so tokens like
 * {{client_name}} can appear in any field.
 */

export interface EmailTemplateInput {
  /** "visual" → wrap structured fields | "html" → use body_html as-is */
...
/** Replace {{token}} occurrences in a string with values from `vars`. */
...
/** Render a template into final {subject, html}, applying interpolation.
