/**
 * ProjectsDashboard — Owner > Projects
 *
 * Two-tab dashboard:
 *   1. Projects — Full 3-row toolbar table page following the CRM/Clients gold standard.
 *   2. Project Builder — Inline multi-step service configurator.
 *
 * Uses real data from `useProjects()` + `useClients()`.
 */
...
/* New Project dialog state is managed at the dashboard level */
...
/* ── Tab config ─────────────────────────────────────────────────── */
const DASHBOARD_TABS = ["Projects", "Project Builder"] as const;
...
export function ProjectsDashboard() {
  ...
  const activeTab = (headerConfig?.activeTab as DashboardTab) ?? "Projects";
  ...
  const dashView = useDashboardView("projects", COLUMNS);
  ...
  // Helper to get client name from nested join
  ...
  const filteredProjects = useMemo(() => {
    ...
    return sorted;
  }, [projects, activeList, searchTerm, sortField, dashView.filters]);
  ...
  // ── Render both tabs (TabPanel keeps them mounted) ───────────────
  return (
    <>
        ...
        <FullBleed>
          ...
          >
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={() =>
                  navigate({ to: `${basePath}/projects/new` as "/owner/projects/new" })
                }
                    ...
                    >
                      <div className="flex flex-col items-center gap-3">
                        ...
                        <p className="text-sm text-foreground-muted">
                          No projects yet — create one to start tracking cycles.
                        </p>
                      </div>
                    </td>
                  </tr>
                ...
                {filteredProjects.map((project) => {
                  ...
                  return (
                    <tr
                      ...
                      onClick={() =>
                        navigate({
                          to: `${basePath}/clients/$clientSlug` as "/owner/clients/$clientSlug",
                          params: {
      ...
      <TabPanel active={activeTab === "Project Builder"} lazy>
        <CreateProjectPage embedded />
      </TabPanel>
    </>
  );
}
