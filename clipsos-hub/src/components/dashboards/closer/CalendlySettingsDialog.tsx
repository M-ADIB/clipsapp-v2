...
export function CalendlySettingsDialog({ open, onOpenChange }: CalendlySettingsDialogProps) {
  ...
  const [eventFilter, setEventFilter] = useState("30 Mins Discovery Call");
  const [showKey, setShowKey] = useState(false);

  // Populate from existing region config
  useEffect(() => {
    if (region) {
      setApiKey(region.calendly_api_key || "");
      setEventFilter(region.calendar_event_filter || "30 Mins Discovery Call");
    }
  }, [region]);
  ...
  const handleSave = async () => {
    if (!apiKey.trim()) {
      toast.error("Please enter your Calendly API key.");
      return;
    }

    try {
      await upsertMutation.mutateAsync({
        calendly_api_key: apiKey.trim(),
        calendar_event_filter: eventFilter.trim() || undefined,
      });
