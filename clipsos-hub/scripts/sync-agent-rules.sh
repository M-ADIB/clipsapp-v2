...
#
# AGENTS.md is the single source of truth for ClipsOS V2.
# This script creates copies for agents that don't read AGENTS.md natively.
#
# Usage:
#   bash scripts/sync-agent-rules.sh
#
# Agents that read AGENTS.md natively (no generated files needed):
#   Codex CLI, OpenCode, Cursor (via pointer), Windsurf (via pointer),
#   Copilot Coding Agent, Roo Code, Aider, Augment Code
#
# Agents with thin pointer files (created manually, not by this script):
...
description: ClipsOS V2 — Multi-tenant agency OS conventions
alwaysApply: true
---"
